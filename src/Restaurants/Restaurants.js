...
class RestaurantCard extends Component{
	...
	render(){
		...
		return(
			...
			<Link to={{
				pathname:`/restaurants/${this.props.restaurant._restaurantIndex}` ,
				state : {
...
class Restaurants extends Component{
	...
	async getBlockchainData(){
    	...
    	for(var i=0; i<result._count;i++){
    		var restaurant = await WE_DO_DELIVERY.methods.getRestaurant(i).call();
    	...
    	this.setState({'restaurants':this.state.restaurants , 'restaurantCount':this.state.restaurantCount ,'loading':false} );
	}
	...
	constructor(props){
		...
		this.listenForEventRestaurantAdded = this.listenForEventRestaurantAdded.bind(this);
		this.changeRestaurantArrayAsGrid = this.changeRestaurantArrayAsGrid.bind(this);
	}
	...
	render(){
	    ...
	    return(
	    		<div className='container'>
	      			{
	      				this.state.loading
						?	<div className='row'><img className='col' src={loader} alt='Loading' /></div>
		      			:  this.changeRestaurantArrayAsGrid().map(function(currentRow,rowIndex){
	      					return (	
	      									...
	      									{currentRow.map(function(currentObj ,index){
								      			...
								      			return (
								      				<div key={index} className='col-3'>
								      					...
								      					<RestaurantCard  restaurant={currentObj}/>
								      				</div>
								      				)
						      				})}
	      								</div>
